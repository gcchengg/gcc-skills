 ...
 */
async function syncHtmlAction(input, options) {
    try {
        ...
        if (options.cover) {
            ...
            try {
                ...
                logger.warning(`Failed to upload cover image: ${error.message}`);
        ...
        if (options.update) {
            ...
            await wechatApi.updateDraft(options.update, options.index || 0, articleData);
            logger.success('Draft updated successfully');
        }
        else {
            logger.info('Creating new draft...');
            const result = await wechatApi.createDraft([articleData]);
...
//# sourceMappingURL=sync-html.js.map