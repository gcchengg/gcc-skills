...
async function uploadCover(page, coverPath) {
  ...
  if (openedMenu) await page.waitForTimeout(400);
  const openedPicker = await clickVisibleText("从图片库选择") || await clickVisibleText("上传封面");
  if (!openedPicker && !await page.locator(".weui-desktop-img-picker").last().isVisible().catch(() => false)) {
    throw new Error("Visible cover image-library entry was not found");
  }

  const picker = page.locator(".weui-desktop-img-picker").last();
  ...
  for (let attempt = 0; attempt < 30 && !cropOpened; attempt += 1) {
    for (const candidate of await page.getByText("下一步", { exact: true }).all()) {
      if (await candidate.isVisible().catch(() => false) && await candidate.isEnabled().catch(() => false)) {
        await candidate.click().catch(() => {});
        await page.waitForTimeout(500);
        if (await page.getByText("上一步", { exact: true }).last().isVisible().catch(() => false)) {
          cropOpened = true;
          break;
        }
      }
