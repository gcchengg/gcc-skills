...
9. If the result is uncertain, record `{"result": "uncertain"}`, inspect the LOFTER profile or drafts read-only, and do not click submit again. If the exact LOFTER URL, observed title, approved manifest digest, and check time match, archive via `resolve_uncertain_publication`. Do not retry, refresh into a second submission, or infer failure from a slow response.
